...
console.log(
  `\n====================合計${tokensArr.length}個のDifyワークフロー=========\n`
);
...
class WorkflowTask extends Task {
  ...
  async run() {
    ...
    try {
      ...
      console.error(
        "DIFY_INPUTSの形式が間違っています。JSON形式であることを確認してください。ワークフローの実行に影響を与える可能性があります"
      );
    }
...
!(async () => {
  if (!tokensArr[0]) {
    $.msg($.name, "【ヒント】まずDifyワークフロートークンを入力してください");
    return;
  }
  let messageList = [];1
