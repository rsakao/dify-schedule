...
export class WorkflowClient extends DifyClient {
  ...
  async getWorkflowResult(input, user, isStream) {
        ...
        return new Promise((resolve, reject) => {
          ...
          try {
            ...
            stream.on('end', async () => {
              ...
              console.log('ワークフロー実行結果の取得', task_id, JSON.stringify(data.outputs))
              let outputs = {}
              if(data.outputs) {
                  try {
                    outputs = JSON.parse(data.outputs)
                  } catch (error) {
                    console.log(`ワークフロー実行結果の取得に失敗:${error}`)
                  }
              }
    ...
    if (!isStream) {
      if (res.data.code) {
        console.log('Difyワークフローの実行失敗', res.data.code, res.data.message)
